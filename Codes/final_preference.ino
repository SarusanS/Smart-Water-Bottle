...
#include <Preferences.h> // Included for permanent storage
...
const float crossArea_dm2 = 0.385; 
...
Preferences prefs; // Global Preferences object
...
void setup() {
  ...
  prefs.begin("water_data", false);
  consumedVolume = prefs.getFloat("consVol", 0.0);
  lastdayconsumbtion = prefs.getFloat("lastDayCons", 0.0);
  volumeLimitLiters = prefs.getFloat("volLimit", 0.0);
  lastvolumesettime = prefs.getULong("limitSetT", 0);
  alreadyreset = prefs.getBool("alrReset", false);
  currentMode = prefs.getInt("curMode", 1);
  ...
  while (millis() - start < SETTING_IDLE_TIMEOUT_MS) {
    ...
    if (raw == HIGH && now - lastDebounceTime > 150) { 
       ...
       if (localTap >= 2) { 
          currentMode = target; 
          consumedVolume = 0; 
          prefs.putInt("curMode", currentMode);
          prefs.putFloat("consVol", 0.0);
  ...
  while (millis() - start < SETTING_IDLE_TIMEOUT_MS) {
    ...
    if (raw == HIGH && now - lastDebounceTime > 150) {
       ...
       if (localR >= 2) { 
          consumedVolume = 0; 
          prefs.putFloat("consVol", 0.0);
          Serial.println("✅ Mode 2 Reset Confirmed"); 
          localR = 0; break; 
       }
       delay(200);
    }
...
void displayLimitSetting() {
  ...
  while (inSettingMode) { 
    ...
    if (settingTouchCount == 2 && (now - touchStart < doubleTouchGap)) { 
      volumeLimitLiters = (float)settingValue; 
      consumedVolume = 0; 
      lastvolumesettime = millis(); 
      confirmed = true; 
      alreadyreset = false; 
      // Save to Preferences
      prefs.putFloat("volLimit", volumeLimitLiters);
      prefs.putFloat("consVol", 0.0);
      prefs.putULong("limitSetT", lastvolumesettime);
      prefs.putBool("alrReset", false);
      break; 
    }
...
void dayreset() {
  if (millis() - lastvolumesettime >= 86400000 && !alreadyreset) { 
    lastdayconsumbtion = consumedVolume; 
    consumedVolume = 0; 
    alreadyreset = true;
    // Save to Preferences
    prefs.putFloat("lastDayCons", lastdayconsumbtion);
    prefs.putFloat("consVol", 0.0);
    prefs.putBool("alrReset", true);
...
void updatevolume() {
  ...
  if (delta_distance > 0.05) { 
    consumedVolume += delta_volume; 
    // Save change immediately
    prefs.putFloat("consVol", consumedVolume);
    Serial.printf("💧 Water consumed. Volume +%.3f dm³ | Total consumed: %.3f dm³\n", delta_volume, consumedVolume); 
  } 
